...
| `searchScope`         | `"level" \| "deep" \| "global"`              | `"level"`        | The field filters and never jumps: typing narrows what the current view shows and leaves the selection and the navigation path untouched, so clearing the query puts the user back exactly where they were. `"level"` filters the level on screen, which is the scope a drill-down implies. `"deep"` matches the level you are on AND everything under it - not the whole tree from wherever you happen to be - so a query typed inside Person cannot answer with fields from Workspaces and make the breadcrumb above the results a lie; each hit renders its ancestor trail under its label, and results are capped at 200. `"global"` is the opt-in to exactly that wider answer, for a tree whose levels are administrative rather than semantic - a country, its regions, its cities - where the user knows the leaf and not the branch it hangs from, and a query is how they say so; the ancestor trail under each hit is what keeps it readable. In `mode="columns"` only the ACTIVE column is filtered. In `mode="tree"` a query already matches at any depth and auto-expands the ancestors of every hit, so `"deep"` is a no-op there and warns in development.                            |
...
| `searchScope="deep"` or `"global"` in `mode="tree"`                                               | A tree query already matches at any depth.                                                                       |
